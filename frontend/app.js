    ...
    books.forEach(book => {
      ...
      if (user.role === 'admin' || user.role === 'moderator') {
        ...
        editBookBtn.addEventListener("click", ()=>{
          ...
          editBookForm.addEventListener('submit', async function(event) {
            ...
            formData.forEach((value, key) => changedata[key] = value.trim())
                await fetch(`http://exam-flask35.std-2588.ist.mospolytech.ru/api/book/${book.book_id}`, {
                    method: "PUT",
                    headers: {
        ...
        deleteBtn.addEventListener("click", async () => {
          await confirmDelete(book.book_id, book.name)
      ...
      for (let i of reviews){
        ...
        <p>Комментарий:${i.comment}</p>
          ...
          if(user.role ==='admin' || user.role ==='moderator'){
              ...
              btnDeleteRew.textContent = 'Удалить'
